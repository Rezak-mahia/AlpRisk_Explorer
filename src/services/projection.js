...
async function fetchJsonFromCoordinateService(url, errorMessage) {
...
export async function convertLV95ToLonLat(x, y) {
  const url =
  ...
  const data = await fetchJsonFromCoordinateService(
    url,
    'Error converting LV95 to WGS84 coordinates'
  )
...
export async function convertLonLatToLV95(lon, lat) {
  const url =
  ...
  const data = await fetchJsonFromCoordinateService(
    url,
    'Error converting WGS84 to LV95 coordinates'
  )
...
export async function convertWebMercatorToLV95(x, y) {
  const lon = (x * 180) / 20037508.34
  const lat =
    (180 / Math.PI) *
    (2 * Math.atan(Math.exp((y / 20037508.34) * Math.PI)) - Math.PI / 2)

  return convertLonLatToLV95(lon, lat)
}